...
package com.example.wlk.zzuar.tf.util;
...
public class BorderedText {
  ...
  public void drawText(final Canvas canvas, final float posX, final float posY, final String text) {
    ...
    Toast.makeText(myapp.getContext(),posX+":"+posY,Toast.LENGTH_SHORT).show();
    canvas.drawText(text, posX, posY, exteriorPaint);
    canvas.drawText(text, posX, posY, interiorPaint);
  }

  /**
   * 此函数对意义应该是在参数的canvas上绘制lines参数里面的string
   * @param canvas 要绘制的canvas
   * @param posX 位置X
   * @param posY　位置Y
   * @param lines 字符串数组
   */
  public void drawLines(Canvas canvas, final float posX, final float posY, Vector<String> lines) {
    ...
    Toast.makeText(myapp.getContext(),"drawLines"+sb.toString(),Toast.LENGTH_SHORT).show();
  }
